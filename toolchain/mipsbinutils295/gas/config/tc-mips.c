      ...
      for (args = insn->args;; ++args)
	{
	  ...
	  switch (*args)
	      ...
	      if (s[0] == '$')
		{
		  ...
		  tmp_s[ie] = '\0';
		  symbol = symbol_find(tmp_s);

		  if (symbol && S_GET_SEGMENT (symbol) == reg_section &&
		      (symval = S_GET_VALUE(symbol), ((symval & RTYPE_GP) || (symval & RTYPE_NUM))))
		    {
		      s += ie;
		      regno = symval & 0x1F;

		      if (regno > 31) /* Sanity check, this should not happen */
			/*as_bad (_("Invalid register number (%d)"), regno);*/
			as_bad ("Invalid register name ($%s)", tmp_s);
		    }
     ...
     char *str;
{
  ...
  if (*str == LP
	...
	;
      if (sp - 4 >= str && sp[-1] == RP)
	{
	  if (isalnum ((unsigned char) sp[-2]))
	    {
	      for (sp -= 3; sp >= str && isalnum ((unsigned char) *sp); sp--)
		;
